...
- 共识层钱包主要用于ROSE代币在共识层内流转，以及质押；（web钱包、插件钱包、bitpie等第三方钱包）
- Emerald钱包用于参与Emerald上的各种生态（MetaMask等可添加RPC网络的第三方钱包（App或插件））
...
Emerald 为 EVM 架构，私钥、钱包地址的算法，均与ETH、BSC 链相同，因此可以直接使用这两个链上的私钥、钱包地址等信息。

因兼容钱包仅支持私钥导入，以下介绍 MetaMask 私钥的获取方式。
