...
> 提示  如果您的验证器节点已经注册并在非哨兵设置中运行，则可以跳过此步骤，因为一旦我们使用新配置重新部署 Oasis 验证器，它将自动更新其在注册表中的地址。
...
> 提示  `SENTRY_CONSENSUS_ID`: 这是哨兵节点的共识ID，base64格式。此 ID 可从以下位置获取`consensus_pub.pem`：
