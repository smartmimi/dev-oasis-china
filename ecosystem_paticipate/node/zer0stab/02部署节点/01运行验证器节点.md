...
[我们建议您通过systemd](https://github.com/systemd/systemd)或[Supervisor](http://supervisord.org/)等进程管理器配置和使用它。
...
`-transactions.fee.gas`和`--transaction.fee.amount`选项取决于网络配置，详见通用[事务标志](https://docs.oasis.dev/general/manage-tokens/advanced/oasis-cli-tools/setup#common-transaction-flags)。
